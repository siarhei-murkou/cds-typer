...
function printEnum(buffer, name, kvs, options = {}, doc=[]) {
    ...
    if (configuration.outputDTsFiles) {
        // In .d.ts, emit an ambient declaration of the enum object shape without an initializer
        buffer.addIndentedBlock(`${opts.export ? 'export ' : ''}declare const ${name}: {`, () =>
            kvs.forEach(([k, v]) => { buffer.add(`readonly ${normalise(k)}: ${v};`) })
        , '}')
