...
class InlineDeclarationResolver {
    ...
    visitElement({name, element, file, buffer = file.classes, modifiers = [], resolverOptions}) {
        ...
        // mark whether this specific property is optional (and not @mandatory) ONLY for CDS types (not entities)
        try {
            const inTypeContext = this.isUnderCdsType(element) === true
            let isOptional = false
            if (inTypeContext) {
                const isArray = Boolean(element?.items)
                const isNullable = isArray ? !element?.notNull : this.visitor.resolver.isOptional(element)
                isOptional = isNullable && !this.visitor.resolver.isMandatory(element)
            }
            type.typeInfo.isOptionalProperty = isOptional
        } catch { /* best-effort; keep backwards compatibility */ }
        this.depth--
        if (this.depth === 0) {
     ...
     * @param {TypeResolveInfo_} type - resolved type info of the property
     * @returns {'?:'|':'}
     */
    getPropertyTypeSeparator(type) {
        return (configuration.propertiesOptional || type?.typeInfo?.isOptionalProperty) ? '?:' : ':'
    }
    ...
    /**
     * Determines whether the passed element is within a CDS 'type' context (not an entity)
     * by walking its parent chain until a definition-kind is found.
     * @param {import('../typedefs').resolver.EntityCSN} element - CSN element to inspect
     * @returns {boolean}
     * @protected
     */
    isUnderCdsType(element) {
        try {
            let node = element
            // Walk up until we find a parent with a kind or run out
            while (node && node.parent) {
                const kind = node.parent.kind
                if (kind && !node.parent.parent) return kind === 'type'
                node = node.parent
            }
        } catch { /* noop */ }
        return false
...
class FlatInlineDeclarationResolver extends InlineDeclarationResolver {
    ...
    flatten({prefix, type, modifiers}) {
        return type.typeInfo.structuredType
            ...
            : [`${this.stringifyModifiers(modifiers)}${normalise(prefix)}${this.getPropertyTypeSeparator(type)} ${this.getPropertyDatatype(type)}`]
    }
...
class StructuredInlineDeclarationResolver extends InlineDeclarationResolver {
    ...
    flatten({fq, type, buffer, modifiers = [], statementEnd = ';'}) {
        ...
        if (type.typeInfo.structuredType) {
            const prefix = fq ? `${this.stringifyModifiers(modifiers)}${normalise(fq)}${this.getPropertyTypeSeparator(type)}`: ''
            buffer.add(`${prefix} {`)
            buffer.indent()
            ...
            buffer.add(`${this.stringifyModifiers(modifiers)}${normalise(fq)}${this.getPropertyTypeSeparator(type)} ${this.getPropertyDatatype(type)}${lineEnding}`)
        }
        this.printDepth--
        return buffer
    }
