...
class InlineDeclarationResolver {
    ...
    visitElement({name, element, file, buffer = file.classes, modifiers = [], resolverOptions}) {
        const enteringRoot = this.depth === 0
        this.depth++
        if (enteringRoot) {
            // remember whether we are in a CDS type context (not entity) for nested elements
            try {
                this.currentIsTypeContext = element?.parent?.kind === 'type'
            } catch { /* noop */ }
        }
        ...
        try {
            const inTypeContext = this.currentIsTypeContext === true
        ...
        if (this.depth === 0) {
            ...
            // reset context when unwinding root
            this.currentIsTypeContext = false
        }
        return type
    }
    ...
    constructor(visitor) {
        ...
        this.depth = 0
        // Tracks whether we are currently resolving/printing within a CDS 'type' (not an entity)
        this.currentIsTypeContext = false
    }
