...
/**
 * Compiles TypeScript source into a .d.ts declaration string using an in-memory TypeScript compiler host.
 * This avoids writing intermediate .ts files to disk.
 * @param {string} tsSource - TypeScript source code
 * @param {string} [virtualFileName] - virtual file name for the TS source (used by TS compiler)
 * @returns {string} the emitted .d.ts content
 */
function emitDtsFromTs(tsSource, virtualFileName = 'index.ts') {
    ...
    host.getSourceFile = (fileName, languageVersion) => {
        ...
        // Fallback to default lib for lib.d.ts requests
        const text = ''
        return ts.createSourceFile(fileName, text, languageVersion, true)
    }
    host.writeFile = (fileName, content) => { outputs[fileName] = content }
    host.fileExists = fileName => fileName === virtualFileName
    host.readFile = fileName => (fileName === virtualFileName ? tsSource : '')
    ...
    const dtsFileName = virtualFileName.replace(/\.ts$/, '.d.ts')
    return outputs[dtsFileName] ?? ''
}
        ...
        sources.map(async source => {
            ...
            try {
                ...
                if (configuration.outputDTsFiles) {
                    // Generate only .d.ts files by compiling TS payload to declarations; do not write .ts or .js files.
                    const tsPayload = source.toTypeDefs()
                    const dtsPayload = emitDtsFromTs(tsPayload, 'index.ts')
                    await fs.writeFile(path.join(dir, 'index.d.ts'), dtsPayload)
                } else {
                    // Default behavior: write TypeScript and accompanying JS exports
                    await Promise.all([
