...
class SourceFile extends File {
    ...
    addOperation(name, parameters, returns, kind, doc, callStyles) {
        ...
        // In .d.ts or when nested inside a namespace, 'declare' is invalid on members; drop it
        const declareKw = configuration.outputDTsFiles ? '' : 'declare '
        this.operations.buffer.addIndentedBlock(`export ${declareKw}const ${opener}`, content, closer)
    ...
    addType(fq, clean, rhs, exportValueLevel = false) {
        ...
        if (exportValueLevel) {
            // In .d.ts files, initializers are not allowed. When outputDTsFiles is enabled,
            // emit a declaration without initializer. Otherwise (index.ts) emit a value with initializer.
            if (configuration.outputDTsFiles) {
                this.types.add(`export declare const ${clean}: ${rhs};`)
            } else {
