...
class Visitor {
    ...
    constructor(csn, options = {}, logger = new Logger()) {
        ...
        // FIXME: this belongs into a keyStorage class (see below where we iterate over all keys)
        /** @type {{[key: string]: CSN[]}} */
        this.keyCache = {}
        ...
        // Going over all keys before starting the visit to ensure isRefNotNull is set in all relevant places.
        // If we don't, whether a property can be nullable can be depenend on the order in which they are 
        // visited in the CSN. (#219)
        // FIXME: this belongs in amendCSN, but that requires external access to #keys
        // -> separate #keys into a keyStorage which is accessible from outside
        for (const entity of Object.values(this.csn.xtended.definitions)) {
            for (const [, key] of this.#keys(entity.name)) {
                if (this.resolver.getMaxCardinality(entity) === 1) {
                    key.isRefNotNull = !!entity.notNull || !!entity.key
                }
            }
        }
    ...
    #keys(name) {
        ...
        return this.keyCache[name] ??= Object.entries({
                        ...
                        for (const [kname, kelement] of this.#keys(element.target)) {
                            if (this.resolver.getMaxCardinality(element) === 1) {  // FIXME: kelement?
                                ...
                                if (Object.hasOwn(entity.elements, foreignKey)) {
                                    ...
                                    kelement.isRefNotNull ??= !!element.notNull || !!element.key //|| !!kelement.key
