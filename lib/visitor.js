...
class Visitor {
    ...
    #aspectify(fq, entity, buffer, options = {}) {
        ...
        if (!configuration.outputDTsFiles) {
            ...
            // In .d.ts: declare the mixin factory type to preserve instance members and static side
            // Build instance member shape as an exported interface and reference it from the factory
            const instanceTypeBuffer = buffer.createSubBuffer()
            const ifaceName = identSingular(clean)
            // Build interface header with proper inheritance: interface X extends A, B, C {}
            const ancestorIfaces = ancestorInfos.map(a => toLocalIdent({ ns: a.namespace, clean: a.entityName, fq: a.fullyQualifiedName }))
            const extendsClause = ancestorIfaces.length ? ` extends ${ancestorIfaces.join(', ')}` : ''
            // Emit interface with instance members
            buffer.addIndentedBlock(`export interface ${ifaceName}${extendsClause} {`, () => {
                const inheritedElements = !isViewOrProjection(entity) ? info.inheritedElements : null
                /** @type {TypeResolveOptions} */
                const resolverOptions = { forceInlineStructs: isEntity(entity) && configuration.inlineDeclarations === 'flat'}
                for (let [ename, element] of Object.entries(entity.elements ?? [])) {
                    if (inheritedElements?.has(ename)) continue
                    const name = ename
                    const { typeName, isOptional, isKey } = this.resolver.resolveAndRequire(element, file, resolverOptions)
                    const keyWrap = isKey ? t => `__.Key<${t}>` : t => t
                    buffer.add(`${name}: ${keyWrap(typeName)}${isOptional ? ' | null' : ''};`)
                }
            }, '}')
            // Per request: do NOT include a constructor signature here. The instance shape is provided
            // via the exported interface, and the class (same name) will pick it up automatically.

            // Build static side shape
            const staticTypeBuffer = buffer.createSubBuffer()
            staticTypeBuffer.add('readonly kind: ' + createUnionOf(...['entity','type','aspect'].map(stringIdent)) + ';')
            staticTypeBuffer.add(`readonly keys: ${createKeysOf(clean)};`)
            staticTypeBuffer.add(`readonly elements: ${createElementsOf(clean)};`)
            // actions type
            const actionsTypeBuffer = buffer.createSubBuffer()
            const actions = Object.entries(entity.actions ?? {})
            if (actions.length) {
                // Construct a type literal with all action signatures
                actionsTypeBuffer.addIndentedBlock('{', () => {
                    /** @type {TypeResolveOptions} */
                    const resolverOptions = { forceInlineStructs: isEntity(entity) && configuration.inlineDeclarations === 'flat'}
                    for (const [aname, action] of actions) {
                        const [, content, ] = SourceFile.stringifyLambda({
                            self: clean,
                            name: aname,
                            parameters: this.#stringifyFunctionParams(action.params, file),
                            returns: action.returns
                                ? this.resolver.resolveAndRequire(action.returns, file, resolverOptions).typeName
                                : 'any',
                            kind: action.kind,
                            doc: docify(action.doc)
                        })
                        // content already includes the signature line(s)
                        actionsTypeBuffer.add(content.join('\n'))
                    }
                }, '}')
            } else {
                actionsTypeBuffer.add('globalThis.Record<never, never>')
            }
            staticTypeBuffer.add(`readonly actions: ${actionsTypeBuffer.join()};`)
            const combined = buffer.createSubBuffer()
            combined.add('{')
            combined.add(instanceTypeBuffer.join())
            combined.add(staticTypeBuffer.join())
            combined.add('}')
            const dtsReturn = `TBase & ${combined.join()}`
            // In .d.ts, avoid 'declare' inside ambient contexts (namespaces). Use plain export function.
            buffer.add(`export function ${identAspect(clean)}<TBase extends new (...args: any[]) => object>(Base: TBase): ${dtsReturn}`)
        }

        // CLASS WITH ADDED ASPECTS
        file.addImport(baseDefinitions.path)
        docify(entity.doc).forEach(d => { buffer.add(d) })
        // In .d.ts, ensure the extends-clause chains ancestor aspects like in .ts output
        const baseExpr = `${baseDefinitions.path.asIdentifier()}.Entity`
        const classExtends = configuration.outputDTsFiles
            ? ancestorInfos
                .reduceRight((wrapped, a) => `${asIdentifier({info: a, wrapper: n => `_${n}Aspect`, relative: file.path})}(${wrapped})`, baseExpr)
            : `${identAspect(toLocalIdent({clean, fq}))}(${baseExpr})`
        buffer.add(`export class ${identSingular(clean)} extends ${classExtends} {${this.#staticClassContents(fq, clean).join('\n')}}`)
        this.contexts.pop()
    }
    ...
    #printEvent(fq, event) {
        ...
        buffer.add('// event')
        // In .d.ts or inside namespace blocks, using 'declare' on members is invalid; omit it
        const declKw = configuration.outputDTsFiles ? '' : 'declare '
        // only declare classes, as their properties are not optional, so we don't have to do awkward initialisation thereof.
        buffer.addIndentedBlock(`export ${declKw}class ${entityName} {`, () => {
            buffer.add(createMember({
                name: 'kind',
